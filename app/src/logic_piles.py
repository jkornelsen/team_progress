...
from app.models import (
    db, Entity, Item, Character, Pile, ItemLimit)
...
def adjust_quantity(item_id, owner_id, delta, position=None, slot=None):
    ...
    logger.debug(
        "adjust_quantity() Item:%s | Owner:%s | Delta:%s | Current:%s",
        item.name, owner_id, delta, pile.quantity)
...
def transfer_item(item_id, from_owner_id, to_owner_id, quantity,
    ...
    # Remove requested amount from source
    adjust_quantity(item_id, from_owner_id, -quantity, position=from_pos)

    # Add to target and capture how much didn't fit
    overflow = adjust_quantity(
        item_id, to_owner_id, quantity, position=to_pos, slot=to_slot)

    # If target full/partially full, put remainder back where it came from
    if overflow > 0:
